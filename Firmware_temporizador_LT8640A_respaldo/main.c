...
int main(void) {
    ...
    while (1) //CICLO INFINITO DE PROGRAMA
    {
        ...
        while (adc_read() < 17 || (in_ignition == 1)) // VOLTAJE SUPERIOR A REFERENCIA Y IGNICION ACTIVA PARA SEGUIR //32
        {
          _delay_ms(300);
		  in_ignition = (PINB & 0X01);
		  PORTB &= ~(1 << VOLT_OUT);
	...
	if (!PORTB & (1 << VOLT_OUT))
		{
            ...
            if (read > 17)  // SI EL ESTADO ACTUAL Y EL ANTERIOR SON MAYORES A LA REFERENCIA ACTUALIZA TIMEOUT //32
            {
                in_ignition = (PINB & 0X01);
