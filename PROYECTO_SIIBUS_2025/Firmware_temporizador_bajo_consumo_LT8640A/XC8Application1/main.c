///                    TEMPORIZADOR PROYECTO SIBUS 2025
...
int main(void) {
    ...
    while (1) //CICLO INFINITO DE PROGRAMA
    {
        ...
        while (adc_read() < 95)  //|| in_ignition == 1) // SE QUEDA ESPERANDO HASTA QUE EL VOLTAJE SEA SUPERIOR AL VOLTAJE DE REFERENCIA //33
        {
          _delay_ms(300);
	    }
        //****************************************************************************
        ...
        {          
            ...
            if (read > 99)  // SI EL ESTADO ACTUAL Y EL ANTERIOR SON MAYORES A LA REFERENCIA ACTUALIZA TIMEOUT //30
            {
                in_ignition = (PINB & 0X01);
                //PORTB |= (1 << VOLT_OUT);
